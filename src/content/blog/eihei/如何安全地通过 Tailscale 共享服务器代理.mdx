...
if [ "${#DOCKER_BRIDGES[@]}" -eq 0 ]; then
  echo "没有检测到 Docker bridge 网卡"
  exit 1
fi

for IFACE in "${DOCKER_BRIDGES[@]}"; do
  if sudo ufw status | grep -Fq "${PORT}/tcp on ${IFACE}"; then
    echo "已存在规则: ${IFACE} -> ${PORT}/tcp"
    continue
  fi

  echo "添加规则: ${IFACE} -> ${PORT}/tcp"
  sudo ufw insert 2 allow in on "${IFACE}" to any port "${PORT}" proto tcp
done
