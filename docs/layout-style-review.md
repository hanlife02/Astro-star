...
- 将 2 处 max-width: 40rem 媒体查询统一为 min-width: 40.001rem 的反向写法
- 将 profile.css 中单独的 72rem 桌面排版断点收敛到通用 56.25rem 桌面断点
- 将 home-layout.css 中 150rem / 200rem 的超宽屏根变量断点合并为单一 150rem 规则
- 删除 `app-panel-card.css` 中无实际内容的空 `72rem` 媒体查询
- 为 collection 页面补充 `collection-page-*` 语义别名类名，并保留旧 `listing-*` 类名兼容
- 为 collection 页面根容器补充 `content-page--collection` 语义态，并保留 `content-page--listing` 兼容
- 为 archive 页面内部结构补充 collection-archive-* 语义别名类名，并保留旧 archive-* 类名兼容
- 为 listing 页面补充 collection-page-header-bar、collection-page-view-all-link、collection-page-title--compact 等语义别名，并补充 content-toc--collection 兼容态
...
- 统一断点体系，并减少 `56.25rem / 56.249rem / 72rem / 150rem / 200rem` 一类散落边界（脚本侧桌面断点已完成常量收敛，CSS 侧 `56.25rem / 56.249rem`、`40rem / 40.001rem` 已完成第一轮统一，`profile.css` 的单点 `72rem` 已完成收敛，`home-layout.css` 的 `150rem / 200rem` 已完成首轮合并）
- 继续收敛命名域，让 collection / archive / listing 的类名层次更统一（已完成 collection 页面首轮语义别名兼容，并补上 `content-page--collection`、`collection-archive-*`、`content-toc--collection` 第二轮兼容层）
- 继续清理剩余无效或噪音布局变量与空媒体查询
