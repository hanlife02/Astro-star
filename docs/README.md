...
`main` 分支中的 `docs/` 或文档工作流发生变化后，主仓库会先检查格式并构建文档，再通知 `hanlife02/Astro-star-docs` 发布 GitHub Pages。若跨仓库通知所需的 `ACTIONS_PAT` 未配置或失效，发布仓库会每 5 分钟检查一次源码提交并自动补发。文档源码只在当前目录维护，发布仓库仅保留部署工作流，站点继续沿用原有域名。
